    ...
    if os.path.exists('Path_with_all_stop.json'):
        ...
        for i in range(len(real)):
            if real[i]['routeId'] in recorded:
                pathdata[real[i]['routeId']]['path'].append(real[i])
            else:
                for j in range(len(route)):
                    if route[j]['Id'] == real[i]['routeId']:
                        ...
                        pathdata[route[j]['Id']] = dictn
                        break

    for i in pathdata.values():
        pathdata[i['routeId']]['path'] = quicksort(i['path'], 'seqNo')
            ...
            for k in range(j['seqNo'] + 1, len(ph)):
                ...
                if length <= 0.00002:
                    ...
                    dic = {'start': ph[j["seqNo"]], \
                           'end': pathdata[j['routeId']]['path'][k], \
                           'routeId': pathdata[j['routeId']]['routeId'], \
                           'from': pathdata[j['routeId']]['departureZh'], \
                           'to': pathdata[j['routeId']]['destinationZh'], \
                           'number': pathdata[j['routeId']]['number'], \
                    ...
                    if pathdata[j['routeId']]['path'][j["seqNo"]]['goBack'] == '1':
                        dic['from'], dic['to'] = dic['to'], dic['from']
                    if pathdata[j['routeId']]['routeId'] not in avapath.keys():
                        avapath[pathdata[j['routeId']]['routeId']] = dic
                    elif (avapath[pathdata[j['routeId']]['routeId']]['seq'] - dic['seq']) > 4:
                        avapath[pathdata[j['routeId']]['routeId']] = dic
                    elif ((dic['seq'] - avapath[pathdata[j['routeId']]['routeId']]['seq']) < 3 and \
                                      (dic['len_start'] + dic['len_end']) <= \
                                          (avapath[pathdata[j['routeId']]['routeId']]['len_start'] + \
                                                   avapath[pathdata[j['routeId']]['routeId']]['len_end'])):
...
@app.route('/cancel/<string:routeId>/<string:stopLocId>', methods=['GET'])
def cancelbell(routeId, stopLocId):
    print '----------http://mysmartbus.herokuapp.com/cancel/%s/%s----------' % routeId, stopLocId
    if not os.path.exists('./belldata.json'):
        belldata_generator()
    bellfile = open("belldata.json", 'r')
    _data = bellfile.read()
